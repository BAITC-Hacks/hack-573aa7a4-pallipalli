"""V3: adaptive Bayesian pilots and joint audience/channel allocation.

Standalone submission: copy this file as agent.py beside local_eval.py.
The learning core matches V2. Final deployment considers all four channels,
conservative call saturation, expected pilot overlap, supported audience splits,
and joint cash/contact/campaign limits with a multistart greedy optimizer.
Only public environment data is used; there is no API or shared-module dependency.
"""
...
class Agent(_LearningAgent):
    """V3: adaptive pilots followed by a joint campaign/channel portfolio.

    This extension is bundled with the complete learning implementation in the
    delivered agent.py. No other project file is needed at submission time.
    The optimizer is an approximation: multiple resource-price greedy starts,
    followed by single-campaign replacement and refill. It does not claim a
    globally optimal integer solution.
    """

    @staticmethod
    def _channel_scale(env, channel, sms_ratio):
        sms_multiplier = float(env.channels["sms"]["conversion_multiplier"])
        multiplier = float(env.channels[channel]["conversion_multiplier"])
        if multiplier > 1.0:
            # A pilot identifies d*p, not d and p separately. For d*p >= 0,
            # min(multiplier*p, 1)/p >= 1 for all 0 < p <= 1. This is a
            # conservative bound for calls, including saturated conversion.
            multiplier = 1.0 if sms_ratio >= 0 else multiplier
        return multiplier / sms_multiplier

    @staticmethod
    def _pilot_distribution(env, cells, current, segment, conservative, beta, candidate_id=None):
        # None represents never contacted, distinct from a contacted zero or
        # negative effect. Sampling probabilities are public-data estimates;
        # actual sampled IDs are intentionally unavailable to this agent.
        distribution = {None: 1.0}
        for cell in cells:
            if cell["current"] != current or cell["segment"] != segment:
                continue
            if cell["samples"] <= 0:
                continue
            ratio = float(cell["mu"])
            if conservative:
                # Lower this proposed offer consistently across prior contacts
                # and deployment, but raise competing prior offers. Lowering
                # competitors would inflate the estimated marginal gain.
                sign = -1.0 if cell["id"] == candidate_id else 1.0
                ratio += sign * beta * math.sqrt(max(0.0, float(cell["var"])))
            exposures = cell.get("exposures", {"sms": cell["inclusion"]})
            for channel, inclusion in exposures.items():
                inclusion = min(1.0, max(0.0, float(inclusion)))
                exposed_ratio = ratio * (float(env.channels[channel]["conversion_multiplier"]) /
                                         float(env.channels["sms"]["conversion_multiplier"]))
                updated = {}
                for previous, probability in distribution.items():
                    updated[previous] = updated.get(previous, 0.0) + probability * (1 - inclusion)
                    best = exposed_ratio if previous is None else max(previous, exposed_ratio)
                    updated[best] = updated.get(best, 0.0) + probability * inclusion
                distribution = updated
        return distribution

    @staticmethod
    def _incremental_ratio(distribution, final_ratio):
        return sum(probability * (final_ratio if previous is None else
                   max(previous, final_ratio) - previous)
                   for previous, probability in distribution.items())

    @staticmethod
    def _audience_groups(frame, id_positions):
        """Only complete audiences expressible with documented filters."""
        groups = [(frame, {})]
        allowed = {"data_segment": {"NON_USER", "LITE", "HEAVY"},
        ...
        for column, values in allowed.items():
            ...
            for value in sorted(values):
                ...
                if len(subset):
                    groups.append((subset, {"filter_" + column: value}))
        if all(column in frame for column in allowed):
            for data in sorted(allowed["data_segment"]):
                for calls in sorted(allowed["call_segment"]):
                    subset = frame[(frame["data_segment"] == data) &
                                   (frame["call_segment"] == calls)]
                    if len(subset):
                        groups.append((subset, {"filter_data_segment": data,
                                                "filter_call_segment": calls}))
        seen = set()
        result = []
        for subset, filters in groups:
            if not 0 < len(subset) <= 5000:
                continue
            mask = 0
            for customer_id in subset["ID_NUMBER"]:
                mask |= 1 << id_positions[customer_id]
            if mask in seen:
                continue
            seen.add(mask)
            result.append({"n": len(subset), "baseline": float(subset["predicted_arpu"].sum()),
                           "mask": mask, "filters": filters})
        return result

    @staticmethod
    def _fit(option, money, contacts, used):
        return (option["cost"] <= money + 1e-8 and option["n"] <= contacts
                and not (option["mask"] & used))

    @staticmethod
    def _portfolio_value(selection):
        return sum(option["net"] for option in selection)

    def _greedy_portfolio(self, options, money, contacts, cash_price=0.0,
                          contact_price=0.0, seed=(), density=False):
        selected = list(seed)
        used = 0
        for option in selected:
            ...
            money -= option["cost"]
            contacts -= option["n"]
        initial_money = max(money, 1.0)
        initial_contacts = max(contacts, 1)
        while len(selected) < 10 and contacts > 0:
            choices = [option for option in options if self._fit(option, money, contacts, used)]
            if not choices:
                break

            def priority(option):
                if density:
                    denominator = (option["cost"] / initial_money +
                                   option["n"] / initial_contacts + 0.1)
                    return option["net"] / denominator
                return option["net"] - cash_price * option["cost"] - contact_price * option["n"]

            choice = max(choices, key=lambda option: (priority(option), option["net"], -option["cost"]))
            if priority(choice) <= 0:
                # Resource prices choose a diverse starting portfolio. Finish
                # using actual positive marginal value within the real limits.
                choice = max(choices, key=lambda option: (option["net"], -option["cost"]))
    ...
    def _select_campaigns(self, env, estimates=None):
        cells = self.cells if estimates is None else estimates
        observed = [cell for cell in cells if cell["samples"] > 0]
        if not observed or "sms" not in env.channels:
            return super()._select_campaigns(env, estimates)
        money = max(0.0, float(env.remaining_budget))
        contacts = max(0, int(env.remaining_contacts))
        beta = float(self.risk_beta)
        id_positions = {customer_id: index for index, customer_id in
                        enumerate(env.customer_profile["ID_NUMBER"])}
        groups_by_cell = {}
        distributions = {}
        options = []
        for cell in observed:
            key = (cell["current"], cell["segment"])
            if key not in groups_by_cell:
                groups_by_cell[key] = self._audience_groups(cell["frame"], id_positions)
                distributions[key] = self._pilot_distribution(env, observed, *key, False, beta)
            mean_ratio = float(cell["mu"])
            conservative_ratio = mean_ratio - beta * math.sqrt(max(0.0, float(cell["var"])))
            if conservative_ratio <= 0:
                continue
            mean_distribution = distributions[key]
            conservative_distribution = self._pilot_distribution(
                env, observed, *key, True, beta, candidate_id=cell["id"])
            for channel in sorted(env.channels):
                unit_cost = float(env.channels[channel]["cost_per_contact"])
                if unit_cost < 0 or not math.isfinite(unit_cost):
                    continue
                mean_final = mean_ratio * self._channel_scale(env, channel, mean_ratio)
                conservative_final = conservative_ratio * self._channel_scale(env, channel, conservative_ratio)
                mean_delta = self._incremental_ratio(mean_distribution, mean_final)
                conservative_delta = self._incremental_ratio(conservative_distribution, conservative_final)
                for group in groups_by_cell[key]:
                    cost = group["n"] * unit_cost
                    net = group["baseline"] * conservative_delta - cost
                    expected_net = group["baseline"] * mean_delta - cost
                    if (group["n"] > contacts or cost > money + 1e-8 or
                            net <= 0 or not math.isfinite(net)):
                        continue
                    campaign = {"filter_current_tariff": cell["current"],
                                "filter_arpu_segment": cell["segment"],
                                "target_tariff": cell["target"], "channel": channel,
                                **group["filters"]}
                    options.append({"cell": cell["id"], "campaign": campaign,
                                    "mask": group["mask"], "n": group["n"],
                                    "baseline": group["baseline"], "cost": cost,
                                    "net": net, "expected_net": expected_net,
                                    "mean_ratio": mean_final,
                                    "conservative_ratio": conservative_final})
        if not options:
            campaigns = super()._select_campaigns(env, estimates)
            self.trace.append({"event": "portfolio_fallback", "reason": "no_positive_conservative_options"})
            return campaigns

        # Identical audiences with more cost and no more estimated incremental
        # value are dominated. Keep the trade-offs; channel choice remains joint.
        buckets = {}
        for option in options:
            buckets.setdefault(option["mask"], []).append(option)
        options = []
        for bucket in buckets.values():
            best_net = -math.inf
            for option in sorted(bucket, key=lambda item: (item["cost"], -item["net"])):
                if option["net"] > best_net + 1e-8:
                    options.append(option)
                    best_net = option["net"]
        options.sort(key=lambda option: (-option["net"], option["cost"], option["cell"],
                                        option["campaign"]["channel"]))
        cash_ratios = sorted(option["net"] / option["cost"] for option in options if option["cost"] > 0)
        contact_ratios = sorted(option["net"] / option["n"] for option in options)

        def prices(values):
            if not values:
                return [0.0]
            return sorted({0.0, *[values[int((len(values) - 1) * q)] * scale
                                for q in (0.25, 0.75) for scale in (0.5, 1.0)]})

        best = []
        pure_greedy = self._greedy_portfolio(options, money, contacts)
        best = pure_greedy
        price_pairs = [(cash_price, contact_price) for cash_price in prices(cash_ratios)
                       for contact_price in prices(contact_ratios)]
        for cash_price, contact_price in price_pairs:
            selection = self._greedy_portfolio(options, money, contacts, cash_price, contact_price)
            if self._portfolio_value(selection) > self._portfolio_value(best) + 1e-8:
                best = selection
        density = self._greedy_portfolio(options, money, contacts, density=True)
        if self._portfolio_value(density) > self._portfolio_value(best) + 1e-8:
            best = density

        # One replacement can release enough budget for multiple valuable
        # smaller campaigns. A bounded refill search improves the greedy starts.
        for _ in range(2):
            improved = best
            for index in range(len(best)):
                seed = best[:index] + best[index + 1:]
                for cash_price, contact_price in price_pairs[::max(1, len(price_pairs) // 5)]:
                    selection = self._greedy_portfolio(options, money, contacts, cash_price,
                                                       contact_price, seed=seed)
                    if self._portfolio_value(selection) > self._portfolio_value(improved) + 1e-8:
                        improved = selection
            if improved is best:
                break
            best = improved
        best.sort(key=lambda option: (-option["net"], option["cost"]))
        used = 0
        campaigns = []
        for index, option in enumerate(best, 1):
            assert not (option["mask"] & used), "portfolio contains overlapping final audiences"
            used |= option["mask"]
            campaign = dict(option["campaign"])
            campaign["campaign_name"] = "v3_%02d_%s_%s_%s" % (
                index, campaign["filter_current_tariff"], campaign["target_tariff"], campaign["channel"])
            campaigns.append(campaign)
            self.trace.append({"event": "portfolio_campaign", "campaign": campaign,
                               "n": option["n"], "cost": option["cost"],
                               "estimated_incremental_net": option["expected_net"],
                               "risk_adjusted_incremental_net": option["net"]})
        self.plan_summary = {
            "variant": "v3_portfolio", "optimizer": "multistart_greedy_with_replacement",
            "candidate_options": len(options), "campaigns": len(campaigns),
            "final_contacts": sum(option["n"] for option in best),
            "final_cost": sum(option["cost"] for option in best),
            "estimated_incremental_net": sum(option["expected_net"] for option in best),
            "risk_adjusted_incremental_net": self._portfolio_value(best),
            "simple_greedy_risk_adjusted_net": self._portfolio_value(pure_greedy),
            "pilot_overlap": "expected inclusion probabilities; actual pilot IDs unavailable",
            "call_response": "conservative bound allowing conversion saturation",
        }
        assert len(campaigns) <= 10
        assert self.plan_summary["final_contacts"] <= contacts
        assert self.plan_summary["final_cost"] <= money + 1e-8
        return campaigns
